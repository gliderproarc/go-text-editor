...
func TestRunner_SpellHighlights_Viewport(t *testing.T) {
    ...
    gotCount := 0
    for _, h := range st.highlights {
        if h.Group != "bg.spell" { continue }
        if h.Start < 0 || h.End > len(text) || h.Start >= h.End { continue }
